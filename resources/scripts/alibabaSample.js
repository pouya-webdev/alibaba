  ...
  (function(){
    function toPFA(n){ return String(n).replace(/\d/g,d=>'۰۱۲۳۴۵۶۷۸۹'[d]); }
    const pad = n => toPFA(String(n).padStart(2,'0'));
    const end = Date.now() + (5*3600 + 32*60 + 48) * 1000;
    ...
    function tick(){
      ...
      hEl.textContent = pad(Math.floor(rem / 3600));
      mEl.textContent = pad(Math.floor((rem % 3600) / 60));
      sEl.textContent = pad(rem % 60);
    }
    tick();
